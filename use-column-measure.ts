...
import { mergeMaxWidths } from './column-layout';
...
/**
 * 原地重测通道（remeasureKey / 自适应列集合变化触发）：
 * 快照全量数据分批离屏重测，全部完成后一次性替换列宽——
 * 期间行保持旧列宽上屏，不下屏、纵向滚动不回顶。
 */
interface RemeasureState<T> {
  entries: GhostEntry<T>[];
  /** 已送测游标（entries 下标，按 MEASURE_BATCH_SIZE 推进） */
  cursor: number;
  /** 各批次累计的最大宽度；完成时整体替换 measuredWidths（合并取 max 无法让列缩窄） */
 ...
 */
export function useColumnMeasure<T>({
  data,
  keyExtractor,
  autoColumns,
  remeasureKey,
  ...
  useEffect(() => {
    ...
    if (dups.size > 0) {
      console.warn(
        `[DataTable] keyExtractor 返回重复 key：${[...dups].map((k) => `"${k}"`).join('、')}。` +
  ...
  const [measuredKeys, setMeasuredKeys] = useState<ReadonlySet<string>>(new Set());
  // 子表列宽已测的行 key（门控：测完再上屏 live 子表；列宽体系重算时作废）
  const [subMeasuredKeys, setSubMeasuredKeys] = useState<ReadonlySet<string>>(new Set());
  // 原地重测通道；null 表示未在重测
  const [remeasure, setRemeasure] = useState<RemeasureState<T> | null>(null);
  ...
  if (prevData !== data) {
    ...
    if (needsMeasure && measuredKeys.size > 0) {
      ...
      if (changed) setMeasuredKeys(next);
    }
  }
  ...
  if (prevAutoKeysSignature !== autoKeysSignature) {
    ...
    setSubMeasuredKeys((prev) => (prev.size > 0 ? new Set<string>() : prev));
    if (needsMeasure && measuredKeys.size > 0) {
      setRemeasure({
  ...
  const { displayData, pending } = useMemo(() => {
    if (!needsMeasure) {
      return { displayData: data, pending: [] as GhostEntry<T>[] };
    }
    const display: T[] = [];
    const pend: GhostEntry<T>[] = [];
    data.forEach((item, i) => {
  ...
  const pendingBatch = useMemo(() => pending.slice(0, MEASURE_BATCH_SIZE), [pending]);

  const handleMeasured = useCallback(
    (batchWidths: Record<string, number>) => {
      // 对比合并：本批最大宽 vs 历史缓存最大宽，取最大值
      setMeasuredWidths((prev) => mergeMaxWidths(prev, batchWidths));
      ...
      setRemeasure((prev) =>
        prev ? { ...prev, widths: mergeMaxWidths(prev.widths, batchWidths) } : prev,
      );
    },
    [pendingBatch],
  );
  ...
  const handleRemeasured = useCallback(
    (batchWidths: Record<string, number>) => {
      if (!remeasure) return;
      const widths = mergeMaxWidths(remeasure.widths, batchWidths);
  ...
  const handleSubMeasured = useCallback((key: string, widths: Record<string, number>) => {
    setMeasuredWidths((prev) => mergeMaxWidths(prev, widths));
  ...
  const invalidateSubMeasured = useCallback((key: string) => {
    setSubMeasuredKeys((prev) => {
      if (!prev.has(key)) return prev;
      const next = new Set(prev);
      next.delete(key);
  ...
  const pendingGhost = useMemo<GhostBatch<T> | null>(
    () =>
      pendingBatch.length > 0
        ...
        : null,
    [pendingBatch, handleMeasured],
  );

  const remeasureGhost = useMemo<GhostBatch<T> | null>(
    () =>
      remeasure
        ? {
        ...
        : null,
    [remeasure, autoKeysSignature, handleRemeasured],
  );
