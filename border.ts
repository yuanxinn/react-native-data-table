...
export function resolveBorder(
  border: boolean | DataTableBorder | undefined,
  defaultLineColor: string,
): ResolvedBorder {
  if (border === false) {
    return { row: null, cell: null, outer: null };
  }
  const cfg: DataTableBorder = border == null || border === true ? {} : border;
  const color = cfg.color ?? defaultLineColor;
  const width = cfg.width ?? StyleSheet.hairlineWidth;
  const horizontal = cfg.horizontal ?? true;
  const vertical = cfg.vertical ?? false;
  const outer = cfg.outer ?? false;
  return {
