...
interface GhostMeasurerProps<T> {
  ...
  columns: { col: TableColumn<T>; key: string }[];
 ...
 */
export function GhostMeasurer<T>({
  entries,
  columns,
  headerTextStyle,
  cellStyle,
  ...
  const headerCheckboxNode = selection?.renderHeaderCheckbox ? (
    selection.renderHeaderCheckbox({ checked: false, indeterminate: false, disabled: false })
  ) : (
    <Checkbox checked={false} onPress={noop} />
  );
  const rowCheckboxNode = (item: T) =>
    selection?.renderCheckbox ? (
      selection.renderCheckbox(false, item)
    ) : (
      <Checkbox checked={false} onPress={noop} />
    );
  ...
  return (
    <View style={measureStyles.ghost} pointerEvents="none">
      {columns.map(({ col, key: colKey }) => {
        const isHost =
          selection != null && String(col.dataIndex) === selection.mergeIntoDataIndex;
        // sorter 列真实表头带排序箭头区，测宽时用相同图标（自定义则用其 null 态真实宽，否则等宽占位）补齐
        const headerContent = (
          <View style={styles.headerContentRow}>
            {renderHeaderContent(col, theme.text, headerTextStyle)}
            {col.sorter
              ? renderSortIcon
                ? renderSortIcon(null)
                : <View style={styles.sortIconPlaceholder} />
              : null}
          </View>
        );
        return (
          <View key={colKey} style={measureStyles.columnStack}>
            <View
              style={[composeHeaderCellStyle({ col, cellStyle }), cellBorderStyle]}
              onLayout={(e) => collect(colKey, `${colKey}:__header__`, e)}
            >
              {isHost
                ? renderSelectionCell({
                    checkbox: headerCheckboxNode,
                    content: headerContent,
                    align: selection?.checkboxAlign,
                  })
                : headerContent}
            </View>
            {entries.map(({ item, index, key }) => {
              const content = renderCellContent(col, item, index, theme.text, cellTextStyle);
              return (
                <View
                  key={key}
                  style={[composeCellStyle({ col, cellStyle, record: { item, index } }), cellBorderStyle]}
                  onLayout={(e) => collect(colKey, `${colKey}:${key}`, e)}
                >
                  {isHost
                    ? renderSelectionCell({
